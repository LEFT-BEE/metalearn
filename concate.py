        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            for task_idx, (support_input, support_target, query_input, query_target) in enumerate(zip(support_inputs, support_targets, query_inputs, query_targets)):
                ...
                for _ in range(args.inner_update_num):
                    ...
                    model.zero_grad()
                    params,list_grad4,list_grad3,list_grad2,list_grad1  = update_parameters(model=model,
                                               loss=inner_loss,scale=args.scale,
                                               params=params,
                                               step_size=step_size,
            ...
            except_mean=(task2+task3+task4)/3





            output4 = cos(task1,except_mean)

            #print(float(output.data[0]))

            f = open(os.path.join(str(args.batch_iter)+args.dataset+'_all_layer_with_exceptmean.txt'), "a")

            f.write(str(output4.item())+ ' \n')
            f.close()
            ...
            f = open(os.path.join(str(args.batch_iter)+args.dataset+'_all_layer_combination.txt'), "a")
            ...
            #print(torch.sign((output)))

            #print((torch.set_printoptions(output)))
            #print((output.data[0].long())*float(torch.sign((output))))

            # print(torch.sign((output.data[0])))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--scale', type=float, help='Fix the head during outer updates.')
