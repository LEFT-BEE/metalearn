...
class notLoss(nn.Module):
    def __init__(self,tempval):
        super(notLoss , self).__init__()
        self.temp = tempval
    def forward(self,logits, target):
        T = self.temp
        batch_size , class_size = logits.shape[0] ,logits.shape[1]
        one_target = torch.nn.functional.one_hot(target)
    
        none_target = 1 - one_target
        non_target = one_target[none_target.bool()].reshape(batch_size , class_size-1 )
        non_target = non_target+ 10 ** (-7)


        pnct_logits = logits[none_target.bool()].reshape(batch_size , class_size-1)

        loss = F.cross_entropy(non_target , pnct_logits)
        return loss
...
def main_val(args, mode, iteration=None):
    ...
    with tqdm(dataloader, total=total, leave=False) as pbar:
        for batch_idx, batch in enumerate(pbar):
            ...
            #For subnet 3
            outer_loss_sub3 = torch.tensor(0. , device = args.device)
            accuracy_sub3 = torch.tensor(0. , device = args.device)
            ...
            for task_idx, (support_input, support_target, query_input, query_target) in enumerate(
                ...
                for _ in range(args.inner_update_num):
                    support_features, support_logit, fixed_support_logit , not_logit = model(support_input, params=params)
                ...
                query_features, query_logit, fixed_query_logit , not_logit = model(query_input, params=params)
                ...
                outer_loss_sub3 = outer_loss_sub3 + criterion_n(not_logit, query_target)
            ...
            # sub_network3
            outer_loss_sub3.div_(args.batch_size)
            loss_not_net.append(outer_loss_sub3)
            #
            ...
            total_loss = outer_loss_sub1 + args.mag*outer_loss_sub2 + args.mag2 * outer_loss_sub3 
    ...
    return loss_logs_sub1, accuracy_logs_sub1, loss_logs_sub2, accuracy_logs_sub2, loss_logs_ensem, accuracy_logs_ensem, loss_not_net
    ...
    with tqdm(dataloader, total=total, leave=False) as pbar:
        for batch_idx, batch in enumerate(pbar):
            ...
            for task_idx, (support_input, support_target, query_input, query_target) in enumerate(
                ...
                params = None
                for _ in range(args.inner_update_num):
                    support_features, support_logit, fixed_support_logit , _ = model(support_input, params=params)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output_folder', type=str, default='./output_not/',
    ...
    parser.add_argument('--mag', type=float, default=0.5,
                        help='multiplier')
    parser.add_argument('--mag2', type=float, default=0.5,
                        help='multiplier2')
    ...
    for iteration in tqdm(range(args.batch_iter)):
        train_loss_logs_sub1, train_accuracy_logs_sub1, train_loss_logs_sub2, train_accuracy_logs_sub2, train_loss_logs_ensem, train_accuracy_logs_ensem , loss_not = main(args=args, mode='meta_train', iteration=iteration)
        val_loss_logs_sub1, val_accuracy_logs_sub1, val_loss_logs_sub2, val_accuracy_logs_sub2, val_loss_logs_ensem, val_accuracy_logs_ensem , val_loss_not = main_val(args=args, mode='meta_valid', iteration=iteration)
        log_pd['train_error'][
    ...
    meta_test_loss_logs, meta_test_accuracy_logs,aa,bb,cc,dd,nl = main_val(args=args, mode='meta_test')
