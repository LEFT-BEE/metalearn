...
def conv3x3_final(in_channels, out_channels, **kwargs):
    return MetaSequential(
        MetaConv2d(in_channels, out_channels, kernel_size=3, padding=1, **kwargs),
        MetaBatchNorm2d(out_channels, track_running_stats=False),
        nn.ReLU(),
...
def conv3x3_second(in_channels, out_channels, **kwargs):
    return MetaSequential(
        MetaConv2d(in_channels, out_channels, kernel_size=3, padding=1, **kwargs),
        MetaBatchNorm2d(out_channels, track_running_stats=False),
        nn.ReLU(),
...
class SeparatedConvNet(MetaModule):
    def __init__(self, in_channels, out_features, hidden_size, wh_size):
        ...
        self.fixed_last = conv3x3_final(hidden_size, hidden_size)
        self.error_last = conv3x3_final(hidden_size , hidden_size)
    ...
    def forward(self, inputs, params=None):
        ...
        features_fixed = self.fixed_last(features, params=get_subdict(params, 'fixed_last'))
        features=F.max_pool2d(features,2)
        features = features.view((features.size(0), -1))

        features_fixed = features_fixed.view((features_fixed.size(0), -1))

        logits = self.classifier(features, params=get_subdict(params, 'classifier'))
        logits_fixed = self.fixed_cls(features_fixed, params=get_subdict(params, 'fixed_cls'))

        return features, logits, logits_fixed
    
class SeparatedConvNet_NOT(MetaModule):
    def __init__(self, in_channels, out_features, hidden_size, wh_size):
        super(SeparatedConvNet_NOT, self).__init__()
        self.in_channels = in_channels
        self.out_features = out_features
        self.hidden_size = hidden_size
        ...
        self.fixed_last = conv3x3_final(hidden_size, hidden_size)
        self.not_last = conv3x3_final(hidden_size , hidden_size)
        
        self.classifier = MetaLinear(hidden_size*wh_size*wh_size, out_features)
        self.fixed_cls = MetaLinear(hidden_size*wh_size*wh_size, out_features)
        self.not_cls = MetaLinear(hidden_size*wh_size*wh_size, out_features)
    ...
    def forward(self, inputs, params=None):
        ...
        features_fixed = self.fixed_last(features, params=get_subdict(params, 'fixed_last'))
        features_not = self.not_last(features , params = get_subdict(params , 'not_last'))
        features=F.max_pool2d(features,2)
        features = features.view((features.size(0), -1))

        features_fixed = features_fixed.view((features_fixed.size(0), -1))
        features_not = features_not.view((features_not.size(0) , -1))

        logits = self.classifier(features, params=get_subdict(params, 'classifier'))
        logits_fixed = self.fixed_cls(features_fixed, params=get_subdict(params, 'fixed_cls'))
        logits_not = self.not_cls(features_not , params = get_subdict(params , 'not_cls'))

        return features, logits, logits_fixed, logits_not


class SeparatedConvNet_more(MetaModule):
    def __init__(self, in_channels, out_features, hidden_size, wh_size):
        super(SeparatedConvNet_more, self).__init__()
        self.in_channels = in_channels
        self.out_features = out_features
        self.hidden_size = hidden_size
        ...
        self.conv3_fixed = conv3x3_second(hidden_size, hidden_size)
    ...
    def forward(self, inputs, params=None):
        ...
        features_conv3 = self.conv3_fixed(features, params=get_subdict(params, 'conv3_fixed'))

        features_conv3 = features_conv3.view((features_conv3.size(0), -1))
        ...
        logits = self.classifier(features, params=get_subdict(params, 'classifier'))
        logits_fixed = self.fixed_cls(features_fixed, params=get_subdict(params, 'fixed_cls'))
        logits_second = self.fixed_cls(features_fixed, params=get_subdict(params, 'fixed_cls_second'))
