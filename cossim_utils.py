...
def update_parameters(model,
    ...
    if isinstance(step_size, (dict, OrderedDict)):
        ...
        for (name, param), grad in zip(params.items(), grads):
            updated_params[name] = param - step_size[name] * grad * scale * (
                        torch.max(torch.abs(grad)))
    ...
    return updated_params, similar_grad4, similar_grad3, similar_grad2,similar_grad1
