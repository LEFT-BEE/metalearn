        ...
        for batch_idx, batch in enumerate(pbar):
            ...
            for task_idx, (support_input, support_target, query_input, query_target) in enumerate(
                ...
                for _ in range(args.inner_update_num):
                    ...
                    inner_loss = F.cross_entropy(support_logit, support_target) + F.cross_entropy(fixed_support_logit, support_target)
                ...
                outer_loss_ensem = outer_loss_ensem+  F.cross_entropy((fixed_query_logit+query_logit)/2, query_target)


                with torch.no_grad():
                    accuracy_sub1 += get_accuracy(query_logit, query_target)
                    accuracy_sub2 += get_accuracy(fixed_query_logit, query_target)
                    accuracy_ensem += get_accuracy((fixed_query_logit+query_logit)/2, query_target)
...
if __name__ == '__main__':
    ...
    result.to_csv(os.path.join(args.output_folder, args.dataset + '_' + args.save_name, 'logs', 'current_result.csv'))
